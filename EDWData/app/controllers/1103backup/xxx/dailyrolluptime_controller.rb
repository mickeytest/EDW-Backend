class DailyrolluptimeController < ApplicationController
  def index
     #\'#{currectTime}\' ")
	  ...
	  rollupdetail=[{"name"=> "OffIVC not started!","color"=> "#C9C9C9","x"=>0,"data"=> [(DateTime.parse((maxrolluotime[0]["maxrollupdate"]).strftime("%Y/%m/%d")+ ' 01:00:00') - 2/24.0).strftime("%Y/%m/%d %H:%M:%S"),(Time.new).strftime("%Y/%m/%d")+ ' 02:00:00']}, 
                    {"name"=> "OffFIN not started!","color"=> "#C9C9C9","x"=>0,"data"=> [(DateTime.parse((maxrolluotime[0]["maxrollupdate"]).strftime("%Y/%m/%d")+ ' 01:00:00') - 2/24.0).strftime("%Y/%m/%d %H:%M:%S"),(Time.new).strftime("%Y/%m/%d")+ ' 02:00:00']},
                    {"name"=> "Rollup not started!","color"=> "#ABABAB","x"=>0,"data"=> [DateTime.parse((maxrolluotime[0]["maxrollupdate"]).strftime("%Y/%m/%d")+ ' 02:00:00').strftime("%Y/%m/%d %H:%M:%S"),(maxrolluotime[0]["maxrollupdate"]).strftime("%Y/%m/%d")+ ' 08:30:00']},
	...
	def  rollupcolourcal(rolluptimestartact,server,rolluptimeendact,rolluptime,currenttime) 
		 ...
		 return case 
			...
			when timediff2 >510 then :"#FC9C12"
