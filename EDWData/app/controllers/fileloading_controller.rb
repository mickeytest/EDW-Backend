		...
		fileloadinghash["EMRDST"]=0
	     ...
	     while i<filecountinformation.length do	
            if	filecountinformation[i]["Server"]=='JAD'	
			    ...
			    if (zeoalltime[3] < filecountinformation[i]["detailtime"] and zeoalltime[1] > filecountinformation[i]["detailtime"])
				      if(c==0)	
					  zeodatastartpoint=i
		 ...
		 if(zeostarttime1!=nil && zeostarttime!=nil)
             if(zeostarttime1>jadstarttime1 and zeostarttime1<jadstarttime)	
			 zeostarttimelast=zeostarttime1
			 zeorollupstarttimelast=zeorollupstarttime1
			 zeofirststarttimelast=zeofirststarttime1
			 zeodatastartpointlast=zeodatastartpoint1
			 zeocurrentlast=zeocurrent1
             else
			 zeostarttimelast=zeostarttime
			 zeorollupstarttimelast=zeorollupstarttime
			 zeofirststarttimelast=zeofirststarttime
			 zeodatastartpointlast=zeodatastartpoint
			 zeocurrentlast=zeocurrent
             end
          else
		     zeostarttimelast=zeostarttime
			 zeorollupstarttimelast=zeorollupstarttime
		 ...
		 if(rolluptimeinfo[0]["Server"]=='JAD' || rolluptimeinfo[0]["Server"]=='EMR')
		      if(zeostarttime!=nil)
		           if(i>=zeorollupstarttimelast && i<zeofirststarttimelast)
				          zeodatacount[i]=0
					elsif(i==zeofirststarttimelast)	 
					    zeodatacount[i]=zeocurrentlast
					elsif(zeocurrentlast>=0 && i>zeofirststarttimelast)	   
					   if(zeocurrentlast>0)
					        inteval=((filecountinformation[zeodatastartpointlast+1]["detailtime"]-filecountinformation[zeodatastartpointlast]["detailtime"])/60.0).round(0)					           
							   zeocurrentlast= zeocurrentlast-alldateinfo[zeodatastartpointlast]				   
					           zeodatacount[i+zeofirstadd]=zeocurrentlast
							   if(inteval!=1)
							         bbbb=1
							           while bbbb<inteval
									      zeofirstadd+=1
               ...
               if(zeostarttime1!=nil)
				   if(i>=zeorollupstarttime1 && i<zeofirststarttime1)
					...
					elsif(zeocurrent1>=0 && i>zeofirststarttime1)	   
					   if(zeocurrent1>0)
					           inteval=((filecountinformation[zeodatastartpoint1+1]["detailtime"]-filecountinformation[zeodatastartpoint1]["detailtime"])/60.0).round(0)					           
							   zeocurrent1= zeocurrent1-alldateinfo[zeodatastartpoint1]				   
					           zeodatacount[i+zeofirstadd]=zeocurrent1
							   if(inteval!=1)
							         bbbb=1
							           while bbbb<inteval
									      zeofirstadd+=1
               ...
               if(zeostarttime!=nil)				
                   if(i>=zeorollupstarttime && i<zeofirststarttime)
					   ...
					   if(zeocurrent>0)
					           ...
					           zeodatacount[i+zeosecondadd]=zeocurrent
							   if(inteval!=1)
							         bbbb=1
							           while bbbb<inteval
									      zeosecondadd+=1
           ...
           if(jadrollupstarttime!=nil) 	  
		       if(i>=jadrollupstarttime && i<jadfirststarttime)
					   ...
					   if(jadcurrent1>0)
					           inteval=((filecountinformation[jaddatastartpoint+1]["detailtime"]-filecountinformation[jaddatastartpoint]["detailtime"])/60.0).round(0)					           
							   jadcurrent1= jadcurrent1-alldateinfo[jaddatastartpoint]				   
					           jaddatacount[i+jadfirstadd]=jadcurrent1
							   if(inteval!=1)
							         bbbb=1
							           while bbbb<inteval
									      jadfirstadd+=1
